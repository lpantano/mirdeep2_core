...
sub CreateOutputMRD{
	...
	$mature=`$scripts/convert_bowtie_output.pl ${name1}_mapped.bwt > ${name1}_mapped.arf`;
    ...
    if($options{'s'}){
        $star=`$scripts/convert_bowtie_output.pl ${name3}_mapped.bwt > ${name3}_mapped.arf`;
	...
	$reads=`$scripts/convert_bowtie_output.pl ${name2}_mapped.bwt > ${name2}_mapped.arf`;
